...
\begin{figure}[H]
    ...
    \caption{Phân tích tìm K tối ưu cho Kmeans.}
    \label{fig:optimal_k}
\end{figure}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Đáng chú ý, cụm màu xanh da trời (Cluster 4) ở góc dưới bên phải trùng khớp vị trí với nhóm \textbf{PUNCT} trong biểu đồ nhãn thật (Hình \ref{fig:2d_pca_true}). 
    \end{itemize}
...
\begin{enumerate}
    ...
    \item Dấu câu (Punctuation) là lớp dễ phân biệt nhất, được tách biệt hoàn toàn bởi phương pháp không giám sát (K-Means) nhưng lại gây khó khăn cho phương pháp có giám sát (LDA).
...
\begin{table}[h!]
    ...
    \caption{So sánh F1-Score chi tiết từng nhãn giữa phương pháp Standard (Std) và Weighted (Wgt) với tỷ lệ chia Train/Test 80/20.}
...
\begin{figure}[H]
    ...
    \caption{Mô hình Naive Bayes với tỉ lệ chia Train/Test 80/20.}
...
\begin{figure}[H]
    ...
    \caption{Mô hình Naive Bayes với tỉ lệ chia Train/Test 70/30.}
    \label{fig:nb_pca_70_30}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/opt_dim_nb_64.png} 
    \caption{Mô hình Naive Bayes với tỉ lệ chia Train/Test 60/40.}
...
\begin{itemize}
    ...
    Một xu hướng được quan sát thấy là dữ liệu càng nhiều, số chiều tối ưu càng giảm. 
    \begin{itemize}
...
Bảng \ref{tab:knn_split_dim_summary} tổng hợp kết quả $R^2$ cao nhất đạt được tương ứng với mỗi cấu hình chia dữ liệu và số chiều.
...
Nhận thấy từ Bảng \ref{tab:knn_split_dim_summary}:
\begin{itemize}
...
Bảng \ref{tab:knn_k_detail} trình bày chi tiết sự biến thiên của MSE và $R^2$ khi thay đổi $K$ từ 1 đến 9 trên tập dữ liệu (80/20).
...
Dựa trên kết quả tối ưu của từng mô hình (KNN tại $K=5$ và MLP), nghiên cứu rút ra các kết luận quan trọng sau:
...
\begin{itemize}
    ...
    \item Với \textbf{MLP}, giảm chiều gây hại đáng kể ($R^2$ giảm từ 0.988 xuống 0.936). Mạng nơ-ron hoạt động như một bộ trích xuất đặc trưng tự động; việc nén dữ liệu tuyến tính bằng PCA trước đó đã làm mất đi các thông tin ngữ nghĩa (nằm ở các chiều phương sai thấp) mà MLP cần để đạt độ chính xác cực đại.
\end{itemize}

\paragraph{3. Đánh giá trực quan qua biểu đồ phân tán (Scatter Plot):}

Để minh chứng rõ hơn cho sự vượt trội về độ chính xác giữa hai mô hình, Hình \ref{fig:regression_scatter_comparison} biểu diễn mối tương quan giữa giá trị xác suất thực tế (Ground Truth - Đường màu đen) và giá trị dự đoán (Prediction - Điểm màu xanh) trên tập kiểm thử.

\begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{images/scatter_knn_true_pred.png} 
        \caption{Biểu đồ phân tán KNN Regression}
        \label{fig:scatter_knn}
\end{figure}
\begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{images/scatter_mlp_true_pred.png} 
        \caption{Biểu đồ phân tán MLP Regression}
        \label{fig:scatter_mlp}
\end{figure}

Quan sát từ biểu đồ cho thấy sự khác biệt về chất lượng dự đoán:
\begin{itemize}
    \item \textbf{Mô hình KNN (Hình \ref{fig:scatter_knn}):} Các điểm dự đoán phân tán rộng xung quanh đường thực tế (dải sai số lớn, màu xanh nhạt lan rộng). Đặc biệt ở vùng chuyển tiếp (xác suất $\approx 0.5$), KNN gặp khó khăn lớn trong việc bám sát đường cong sigmoid của dữ liệu gốc, dẫn đến hiện tượng dự đoán bị nhiễu nhiều.
    \item \textbf{Mô hình MLP (Hình \ref{fig:scatter_mlp}):} Các điểm dự đoán tập trung dày đặc và bám rất sát đường cong thực tế (đường màu đen gần như nằm giữa đám mây điểm). Dải sai số (vùng màu xanh) hẹp hơn đáng kể so với KNN. Điều này xác nhận trực quan rằng MLP đã học được hàm ánh xạ trơn tru và chính xác hơn nhiều, giảm thiểu tối đa các điểm dự đoán ngoại lai.
\end{itemize}
