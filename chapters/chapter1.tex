...
\subparagraph{Sơ lược bài toán:} Nhận diện từ loại (Part-of-Speech - POS Tagging)~\cite{definition_pos_tagging} là một nhiệm vụ cơ bản và quan trọng bậc nhất trong lĩnh vực Xử lý Ngôn ngữ Tự nhiên (NLP). Việc xác định đúng từ loại là bước nền tảng để máy tính có thể "hiểu" được cấu trúc và ngữ nghĩa của câu. Nếu không biết từ nào là danh từ, từ nào là động từ, máy tính không thể phân tích được ai đang làm gì (chủ ngữ - vị ngữ).

\subparagraph{Mục tiêu bài toán:} Gán cho mỗi từ (hoặc cụm từ) trong câu một nhãn (tag) tương ứng với vai trò ngữ pháp của nó. Các nhãn này chính là các "từ loại" quen thuộc ví dụ như:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Nghiên cứu Tối ưu hóa Đặc trưng:} Khám phá ảnh hưởng của kỹ thuật giảm chiều dữ liệu (ví dụ: PCA) lên không gian đặc trưng của PhoBERT. Mục tiêu là phân tích sự cân bằng (trade-off) giữa độ chính xác của mô hình và hiệu quả tính toán (giảm thời gian huấn luyện, giảm độ phức tạp).
