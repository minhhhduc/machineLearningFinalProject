...
K-Nearest Neighbor (KNN) là một thuật toán học máy đơn giản nhưng hiệu quả, thường được sử dụng cho các bài toán phân loại và hồi quy. 
Ý tưởng chính của KNN là dựa vào sự tương đồng giữa các mẫu dữ liệu để đưa ra dự đoán cho một mẫu mới. Đặc biệt, KNN 
không yêu cầu quá trình huấn luyện phức tạp, mà chỉ lưu trữ toàn bộ dữ liệu huấn luyện và có thể sử dụng chúng trực tiếp trong quá trình dự đoán.

\paragraph{Hàm dự đoán của KNN:} 
$$F(x_i, k) = \left\{   
\begin{matrix}
    max\limits_c\left(\left|\left\{j : j \in M\left(x_i, k\right) \cap y_j = c\right\}\right|\right)_{c=1}^C &, \text{phân loại}\\
    \frac{1}{k}\sum_{j \in M\left(x_i, k\right)} y_j &, \text{hồi quy}
\end{matrix}\right.
$$

Trong đó:
\begin{itemize}
    \item $M(x_i, k)$ là tập hợp $k$ điểm dữ liệu trong tập huấn luyện gần nhất với điểm $x_i$ theo hàm khoảng cách đã chọn.
    \item $C$ là số lớp trong bài toán phân loại.
\end{itemize}
...
\end{itemize}
\textit{Giải thích:}  
KNN sử dụng các hàm khoảng cách để đo lường sự tương đồng giữa các điểm
